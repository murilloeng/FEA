...
namespace fea
{
	namespace mesh
	{
		...
		{
			//constructor
			Triangle10::Triangle10(void) : Triangle(3)
			{
				return;
			}

			//destructor
			Triangle10::~Triangle10(void)
			{
				return;
			}

			//geometry
			uint32_t Triangle10::vertices(void) const
			{
				return 10U;
			}

			//integration
			void Triangle10::function(double* N, const double* p) const
			{
				//data
				const double r = p[0];
				const double s = p[1];
			...
			void Triangle10::gradient(double* B, const double* p) const
			{
				//data
				const double r = p[0];
				const double s = p[1];
