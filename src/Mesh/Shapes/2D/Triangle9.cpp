...
namespace fea
{
	namespace mesh
	{
		...
		{
			//constructor
			Triangle9::Triangle9(void) : Triangle(3)
			{
				return;
			}

			//destructor
			Triangle9::~Triangle9(void)
			{
				return;
			}

			//geometry
			uint32_t Triangle9::vertices(void) const
			{
				return 9U;
			}

			//integration
			void Triangle9::function(double* N, const double* p) const
			{
				//data
				const double r = p[0];
				const double s = p[1];
			...
			void Triangle9::gradient(double* B, const double* p) const
			{
				//data
				const double r = p[0];
				const double s = p[1];
